...
export function ChatApp() {
  ...
  function patchMessage(chatId: string, messageId: string, patch: Partial<ChatMessage>) {
    setMessagesByChat((prev) => ({
      ...prev,
  ...
  function sendMessage() {
    ...
    const reply = ASSISTANT_REPLIES[replyIndexRef.current % ASSISTANT_REPLIES.length];
    ...
    timersRef.current.delay = setTimeout(() => {
      ...
      appendMessage(chatId, { id: messageId, role: 'assistant', content: words[0] });

      timersRef.current.typing = setInterval(() => {
        shown += 1;
        patchMessage(chatId, messageId, { content: words.slice(0, shown).join(' ') });
